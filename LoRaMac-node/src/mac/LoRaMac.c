 ...
 */
#ifdef _COSMIC_
@near static uint8_t LoRaMacBuffer[LORAMAC_PHY_MAXPAYLOAD];
#else
 ...
 */
#ifdef _COSMIC_
@near static uint8_t LoRaMacRxPayload[LORAMAC_PHY_MAXPAYLOAD];
#else
 ...
 */
#ifdef _COSMIC_
@near static uint8_t MacCommandsBuffer[LORA_MAC_COMMAND_MAX_LENGTH];

/*!
 * Buffer containing the MAC layer commands which must be repeated
 */
@near static uint8_t MacCommandsBufferToRepeat[LORA_MAC_COMMAND_MAX_LENGTH];
#else
...
printf("appskey: ");
for(i=0; i<16; i++) {
    printf("%02x ", (uint16_t)LoRaMacAppSKey[i]);
}
printf("\r\n"); 
        ...
        case FRAME_TYPE_DATA_UNCONFIRMED_DOWN:
            {
                ...
                if( sequenceCounterDiff < ( uint16_t )( 1 << 15 ) )
                {
...
LoRaMacStatus_t PrepareFrame( LoRaMacHeader_t *macHdr, LoRaMacFrameCtrl_t *fCtrl, uint8_t fPort, void *fBuffer, uint16_t fBufferSize )
{
    ...
    switch( macHdr->Bits.MType )
    {
        case FRAME_TYPE_JOIN_REQ:
            ...
            LoRaMacDevNonce = Radio.Random( ) + randr(0, 0xFFFF);
